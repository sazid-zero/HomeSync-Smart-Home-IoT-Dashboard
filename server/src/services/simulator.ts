  ...
  intervalId = setInterval(async () => {
    try {
      ...
      for (const user of users) {
        ...
        const readings: { device_id: string; reading_type: string; value: number; unit: string }[] = [];
        const powerUpdates: { id: string; consumption: number }[] = [];
        ...
        for (const device of devices) {
          ...
          if (device.is_on) {
            ...
            if (device.type.toLowerCase().includes('ac') || device.name.toLowerCase().includes('condition')) basePowerKw = 2.5;
            if (device.type.toLowerCase().includes('heater')) basePowerKw = 2.0;
            ...
            if (device.type.toLowerCase().includes('tv') || device.type.toLowerCase().includes('monitor')) basePowerKw = 0.15;
